	...
	.directive('rnCarouselControls', [function() {
		return {
			...
			template: '<div class="rn-carousel-controls">' +
				'<span class="rn-carousel-control rn-carousel-control-prev" ng-click="prev()" ng-if="index > 0">' +
					'<span class="glyphicon glyphicon-chevron-left"></span>' +
				'</span>' +
				'<span class="rn-carousel-control rn-carousel-control-next" ng-click="next()" ng-if="index < items.length - 1">' +
...
angular.module('tApp')
	.directive('rnCarouselIndicators', [function() {
		return {
			...
			template: '<div ng-show="items && items.length > 1" class="rn-carousel-indicator">' +
		...
		function($swipe, $window, $document, $parse, $compile, $timeout) {
			...
			return {
				...
				controller: function($scope,$element,$attrs,$transclude){
					...
					// complete hack
					// allow refresh of carousel dimentions
					// after initial load
					// to double check that container
					// was not in animation initially
					this.registerCarouselControls = function (controls){
